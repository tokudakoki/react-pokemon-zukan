...
import { fetchPokemonDetail, type PokemonDetail as PokemonDetailResult } from '../api/pokemonDetail';
...
const PokemonDetail: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery<PokemonDetailResult>({
  ...
  if (!isValidId) return <div>ポケモンが見つかりません</div>;
  ...
  const totalBaseStats = data.baseStats.reduce(
    (sum: number, stat: PokemonDetailResult['baseStats'][number]) => sum + stat.value,
    0
  );
    ...
    <div className="p-4 max-w-[400px] m-auto">
      ...
      <div className="mt-4 bg-white shadow-md rounded p-8 flex flex-col items-center gap-4">
        ...
        <div className="mt-4 grid grid-cols-1 gap-x-2 w-full">
          {data.baseStats.map((stat: PokemonDetailResult['baseStats'][number]) => (
          ...
          <div className="flex items-center">
            ...
            <div className="flex-1 bg-gray-200 rounded-full h-4">
              <div
                className="bg-blue-600 rounded-full h-4"
                style={{ width: `${(totalBaseStats / 780) * 100}%` }}
              ></div>
            </div>
            <span className="ml-2 w-8">{totalBaseStats}</span>
          </div>
        </div>
      </div>
